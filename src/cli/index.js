...
class LangObservatoryCLI {
  ...
  async startServices(options) {
    ...
    if (options.port)
      config.api = { ...config.api, port: parseInt(options.port) };
    ...
    if (options.api !== false) {
      ...
      await server.start();
      this.logger.info(
        `API server started on ${config.api?.host || '0.0.0.0'}:${config.api?.port || 3000}`
      );
    }
  ...
  async testLLMCall(options) {
    ...
    this.logger.info(
      `Testing LLM call with ${options.provider}/${options.model}...`
    );
    ...
    console.log(
      ...
      JSON.stringify(result.neuromorphicInsights, null, 2)
    );
  ...
  async planTasks(options) {
    ...
    plan.quantumPlan.phases.forEach((phase, index) => {
      ...
      phase.tasks.forEach(taskInfo => {
        console.log(
          `  - ${taskInfo.task.id} (priority: ${taskInfo.task.priority}, duration: ${taskInfo.task.estimatedDuration}ms)`
        );
      });
    });
  ...
  async displayMetrics(options) {
    ...
    if (options.format === 'json') {
      ...
      console.log(
        `Processing Efficiency: ${(neuromorphicMetrics.processingEfficiency * 100).toFixed(1)}%`
      );
      console.log(
        `Average Confidence: ${(neuromorphicMetrics.averageConfidence * 100).toFixed(1)}%`
      );
      console.log(`Total Calls Processed: ${neuromorphicMetrics.totalCalls}`);

      console.log('\n=== Photon Processor Stats ===');
      console.log(
        `Total Photons Processed: ${photonStats.totalPhotonsProcessed}`
      );
      console.log(
        `Average Wavelength: ${photonStats.averageWavelength.toFixed(2)}nm`
      );
      console.log(
        `Processing Rate: ${photonStats.processingRate.toFixed(2)} photons/sec`
      );
    }
  ...
  displayStatusTable(status) {
    ...
    Object.entries(status.services).forEach(([service, serviceStatus]) => {
      ...
      console.log(
        `  ${indicator} ${service}: ${serviceStatus.healthy ? 'healthy' : 'unhealthy'}`
      );
    });
    ...
    if (status.reliability) {
      console.log('\nReliability:');
      console.log(
        `  Circuit Breakers: ${status.reliability.overallHealthy ? 'healthy' : 'degraded'}`
      );
    }
    ...
    if (status.performance) {
      console.log('\nPerformance:');
      console.log(
        `  Current Load: ${(status.performance.metrics.currentLoad * 100).toFixed(1)}%`
      );
      console.log(
        `  Auto Scaling: ${status.performance.autoScaling.enabled ? 'enabled' : 'disabled'}`
      );
    }
