...
class ResourcePooler extends EventEmitter {
    ...
    async initialize() {
        ...
        try {
            ...
            this.logger.info(`Resource Pooler initialized with ${this.available.length} resources`);
    ...
    async acquire(timeout = this.config.acquireTimeout) {
        ...
        try {
            ...
            this.stats.timeouts++;
            this.logger.warn(`Resource acquisition failed after ${Date.now() - startTime}ms:`, error);
            throw error;
        }
    }
    ...
    async release(resource) {
        ...
        try {
            ...
            if (!isValid) {
                this.logger.warn('Invalid resource detected during release, destroying');
    ...
    notifyWaitingRequest() {
        ...
        const [requestId, pendingRequest] = Array.from(this.pending.entries())
        ...
        for (let i = 0; i < scaleAmount; i++) {
            createPromises.push(
                this.createResource().then(resource => {
                    ...
                    this.logger.error('Failed to create resource during scale up:', error);
                })
            );
        }
    ...
    async performHealthCheck() {
        ...
        for (const resource of [...this.available]) {
            healthCheckPromises.push(
                this.validateResource(resource).then(isValid => {
                    if (!isValid) {
                        const index = this.available.indexOf(resource);
                        if (index !== -1) {
                            this.available.splice(index, 1);
                    ...
                    this.logger.error(`Health check failed for resource ${resource.id}:`, error);
                })
            );
        }
    ...
    async performAdaptiveScaling() {
        ...
        const avgUtilization = this.scalingMetrics.utilizationSamples
            .reduce((sum, sample) => sum + sample.utilization, 0) / 
        ...
        if (timeSinceLastScale < 60000) { // 1 minute cooldown
            return;
        }
    ...
    recordAcquisition(duration) {
        ...
        // Update average
        const totalTime = this.scalingMetrics.acquisitionTimes
            .reduce((sum, time) => sum + time, 0);
        this.stats.averageAcquisitionTime = totalTime / this.scalingMetrics.acquisitionTimes.length;
    }
