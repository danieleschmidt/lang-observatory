...
class MetricsManager {
    ...
    recordError(operation, duration, error, traceId) {
        ...
        if (metric) {
            ...
            this._updateAggregatedMetrics(operation, { success: false, duration, error });
            this.operationMetrics.delete(key);
        }
    ...
    async recordLLMUsage(callData) {
        ...
        this.logger.debug(`Recorded LLM usage for ${key}: $${callData.cost?.total || 0}`);
    }

    getOperationMetrics(operation) {
        const aggregated = this._getAggregatedMetrics(operation);
        const active = Array.from(this.operationMetrics.values())
            .filter(m => m.operation === operation && m.status === 'active');
    ...
    getCostAnalysis(timeRangeHours = 24) {
        ...
        for (const metrics of this.llmUsageMetrics.values()) {
            ...
            const recentCost = recentCalls.reduce((sum, call) => sum + call.cost.total, 0);
    ...
    _getAggregatedMetrics(operation) {
        const operationMetrics = Array.from(this.aggregatedMetrics.values())
        ...
        const totals = operationMetrics.reduce((acc, m) => ({
        ...
        return {
            ...totals,
            successRate: totals.totalCalls > 0 ? totals.successCalls / totals.totalCalls : 0,
            errorRate: totals.totalCalls > 0 ? totals.errorCalls / totals.totalCalls : 0,
            avgDuration: totals.totalCalls > 0 ? totals.totalDuration / totals.totalCalls : 0
        };
    }
    ...
    _generateHourlyAggregates() {
        ...
        const hourlyMetrics = Array.from(this.aggregatedMetrics.values())
            .filter(m => m.hour === currentHour);

        if (hourlyMetrics.length > 0) {
            this.logger.debug(`Generated aggregates for ${hourlyMetrics.length} operations in hour ${currentHour}`);
        }
    }
