...
class LangfuseTracer {
  ...
  async initialize() {
    ...
    if (!this.config.publicKey || !this.config.secretKey) {
      this.logger.warn(
        'Langfuse credentials not provided, running in mock mode'
      );
  ...
  endTrace(traceId, result = {}) {
    ...
    this.logger.debug(
      `Ended trace ${traceId}: ${trace.status} (${trace.duration}ms)`
    );
  }
  ...
  async recordLLMCall(callData) {
    ...
    this.logger.debug(
      `Recorded LLM call: ${callData.provider}/${callData.model}`
    );
  }
  ...
  _calculateTokens(callData) {
    ...
    const inputTokens =
      typeof callData.input === 'string' ? callData.input.length / 4 : 0;
    const outputTokens =
      typeof callData.output === 'string' ? callData.output.length / 4 : 0;
  ...
  _calculateCost(callData) {
    ...
    return {
      ...
      total:
        tokens.input * costPerToken.input + tokens.output * costPerToken.output,
    };
  }
