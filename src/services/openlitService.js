...
class OpenLITCollector {
  ...
  async recordLLMMetrics(callData) {
    ...
    this.logger.debug(
      `Recorded ${metrics.length} LLM metrics for ${callData.provider}/${callData.model}`
    );
  ...
  async _export() {
    ...
    try {
      ...
      this.logger.debug(
        `Exported ${metricsToExport.length} metrics to OTEL collector`
      );
    } catch (error) {
  ...
  async _exportToOTEL(metrics) {
    ...
    return new Promise(resolve => {
      setTimeout(() => {
        this.logger.debug(
          `Submitted ${metrics.length} metrics to OTEL endpoint: ${this.config.endpoint}`
        );
        resolve();
      }, 50);
    });
