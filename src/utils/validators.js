...
class Validators {
  static validateLLMCallData(data) {
    ...
    if (
      ...
      errors.push({
        field: 'output',
        message: 'Output must be a string or object',
      });
    }
    ...
    if (data.tokens && typeof data.tokens === 'object') {
      if (
        ...
        !Number.isInteger(data.tokens.input)
      ) {
        errors.push({
          field: 'tokens.input',
          message: 'Input tokens must be an integer',
        });
      }

      if (
        data.tokens.output !== undefined &&
        !Number.isInteger(data.tokens.output)
      ) {
        errors.push({
          field: 'tokens.output',
          message: 'Output tokens must be an integer',
        });
      }

      if (
        data.tokens.total !== undefined &&
        !Number.isInteger(data.tokens.total)
      ) {
        errors.push({
          field: 'tokens.total',
          message: 'Total tokens must be an integer',
        });
      }
    }
    ...
    if (data.cost && typeof data.cost === 'object') {
      if (
        data.cost.input !== undefined &&
        typeof data.cost.input !== 'number'
      ) {
        errors.push({
          field: 'cost.input',
          message: 'Input cost must be a number',
        });
      }

      if (
        data.cost.output !== undefined &&
        typeof data.cost.output !== 'number'
      ) {
        errors.push({
          field: 'cost.output',
          message: 'Output cost must be a number',
        });
      }
  ...
  static validateOperationName(operation) {
    ...
    if (operation.length < 1 || operation.length > 100) {
      throw new ValidationError(
        'Operation name must be between 1 and 100 characters'
      );
    }

    if (!/^[a-zA-Z0-9_\-.]+$/.test(operation)) {
      throw new ValidationError(
        'Operation name can only contain alphanumeric characters, underscores, hyphens, and dots'
      );
    }
  ...
  static validateMetricData(data) {
    ...
    if (data.type && !['counter', 'gauge', 'histogram'].includes(data.type)) {
      errors.push({
        field: 'type',
        message: 'Metric type must be one of: counter, gauge, histogram',
      });
    }
    ...
    if (
      data.timestamp &&
      (!Number.isInteger(data.timestamp) || data.timestamp < 0)
    ) {
      errors.push({
        field: 'timestamp',
  ...
  static validateConfiguration(config) {
    ...
    if (config.langfuse) {
      if (config.langfuse.host && typeof config.langfuse.host !== 'string') {
        errors.push({
          field: 'langfuse.host',
          message: 'Langfuse host must be a string',
        });
      }

      if (
        config.langfuse.publicKey &&
        typeof config.langfuse.publicKey !== 'string'
      ) {
        errors.push({
          field: 'langfuse.publicKey',
          message: 'Langfuse public key must be a string',
        });
      }

      if (
        config.langfuse.secretKey &&
        typeof config.langfuse.secretKey !== 'string'
      ) {
        errors.push({
          field: 'langfuse.secretKey',
          message: 'Langfuse secret key must be a string',
        });
      }
    }
    ...
    if (config.openlit) {
      if (
        config.openlit.endpoint &&
        typeof config.openlit.endpoint !== 'string'
      ) {
        errors.push({
          field: 'openlit.endpoint',
          message: 'OpenLIT endpoint must be a string',
        });
      }

      if (
        config.openlit.serviceName &&
        typeof config.openlit.serviceName !== 'string'
      ) {
        errors.push({
          field: 'openlit.serviceName',
          message: 'OpenLIT service name must be a string',
        });
      }
    }
    ...
    if (config.metrics) {
      if (
        config.metrics.retentionDays !== undefined &&
        (!Number.isInteger(config.metrics.retentionDays) ||
          config.metrics.retentionDays < 1)
      ) {
        errors.push({
          field: 'metrics.retentionDays',
          message: 'Metrics retention days must be a positive integer',
        });
      }

      if (
        config.metrics.aggregationInterval !== undefined &&
        (!Number.isInteger(config.metrics.aggregationInterval) ||
          config.metrics.aggregationInterval < 1000)
      ) {
        errors.push({
          field: 'metrics.aggregationInterval',
          message: 'Metrics aggregation interval must be at least 1000ms',
        });
      }
    }
    ...
    if (typeof input === 'object' && input !== null) {
      ...
      for (const [key, value] of Object.entries(input)) {
        if (typeof key === 'string' && key.length <= 100) {
          sanitized[this.sanitizeInput(key, 100)] = this.sanitizeInput(
            value,
            maxLength
          );
        }
