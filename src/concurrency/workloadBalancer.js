...
class WorkloadBalancer extends EventEmitter {
  ...
  async initialize() {
    try {
      ...
      this.logger.info(
        `Workload Balancer initialized with ${this.config.algorithm} algorithm`
      );
  ...
  async drainWorker(workerId, timeout = 30000) {
    ...
    if (worker.currentLoad > 0) {
      this.logger.warn(
        `Worker ${workerId} still has ${worker.currentLoad} active requests after drain timeout`
      );
    }
  }
  ...
  async processRequest(request, options = {}) {
    ...
    try {
      ...
      // Retry logic
      if (
        options.retryCount < this.config.retryAttempts &&
        this.shouldRetry(error)
      ) {
        await this.delay(
          this.config.retryDelay * Math.pow(2, options.retryCount)
        ); // Exponential backoff
  ...
  selectAdaptive(workers) {
    ...
    const scoredWorkers = workers.map(worker => {
      ...
      const recentRequests = metrics
        ? metrics.requestCounts
            .slice(-10)
            .reduce((sum, count) => sum + count, 0)
        : 0;
      const recentErrors = metrics
        ? metrics.errorCounts.slice(-10).reduce((sum, count) => sum + count, 0)
        : 0;
  ...
  recordRequestSuccess(workerId, responseTime) {
    ...
    if (worker && metrics) {
      ...
      worker.averageResponseTime =
        (worker.averageResponseTime * (worker.totalRequests - 1) +
          responseTime) /
        worker.totalRequests;
  ...
  recordRequestFailure(workerId, responseTime, error) {
    ...
    if (worker && metrics) {
      ...
      if (errorRate > 0.5 && worker.totalRequests > 10) {
        ...
        this.logger.warn(
          `Worker ${workerId} marked as unhealthy due to high error rate: ${errorRate}`
        );
  ...
  async performHealthChecks() {
      ...
      async ([workerId, worker]) => {
        try {
          ...
          if (previousStatus !== worker.status) {
            this.logger.info(
              `Worker ${workerId} status changed: ${previousStatus} -> ${worker.status}`
            );
          ...
          this.logger.error(
            `Health check failed for worker ${workerId}:`,
            error
          );
          worker.status = 'unhealthy';
        }
    ...
    this.stats.failedWorkers = Array.from(this.workers.values()).filter(
      ...
      workerStats[workerId] = {
        ...
        errorRate:
          worker.totalRequests > 0
            ? worker.totalErrors / worker.totalRequests
            : 0,
  ...
  async shutdown() {
    ...
    // Drain all workers
    const drainPromises = Array.from(this.workers.keys()).map(
      workerId => this.drainWorker(workerId, 10000) // 10 second timeout
    );

    await Promise.allSettled(drainPromises);

    // Shutdown circuit breakers
    const shutdownPromises = Array.from(this.circuitBreakers.values()).map(cb =>
      cb.shutdown()
    );
