...
const schemas = {
    llmCall: {
        ...
        status: { required: false, type: 'string', enum: ['success', 'error', 'timeout'] },
        error: { required: false, type: 'string', maxLength: 1000 },
        metadata: { required: false, type: 'object' },
        traceId: { required: false, type: 'string', pattern: /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i },
    ...
    trace: {
        ...
        parentTraceId: { required: false, type: 'string', pattern: /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i },
        metadata: { required: false, type: 'object' },
    ...
    metric: {
        ...
        type: { required: false, type: 'string', enum: ['counter', 'gauge', 'histogram'] },
    ...
    query: {
        ...
        groupBy: { required: false, type: 'string', enum: ['hour', 'day', 'week', 'month', 'provider', 'model'] }
    }
};
...
function validateField(value, rules, fieldName) {
    ...
    if (Array.isArray(value)) {
        ...
        if (rules.items) {
            const itemSchema = typeof rules.items === 'string' ? schemas[rules.items] : rules.items;
            value.forEach((item, index) => {
                const itemErrors = validateObject(item, itemSchema, `${fieldName}[${index}]`);
