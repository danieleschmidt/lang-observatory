...
class AutoScalingManager extends EventEmitter {
  constructor(config = {}) {
    ...
    this.scaling = {
      horizontal: {
        ...
        maxInstances:
          config.horizontal?.maxInstances || Math.max(os.cpus().length, 4),
  ...
  async initializeMaster() {
    ...
    this.logger.info(
      `Auto-scaling Manager initialized with ${this.clusterState.actualWorkers} workers`
    );
  }
  ...
  async collectClusterMetrics() {
    ...
    clusterMetrics.errorRate =
      clusterMetrics.totalRequests > 0
    ...
    Object.keys(this.metrics).forEach(key => {
      if (
        Array.isArray(this.metrics[key]) &&
        this.metrics[key].length > maxHistorySize
      ) {
  ...
  calculateScalingDecision(metrics) {
    ...
    const now = Date.now();
    if (
      now - this.scaling.horizontal.lastAction <
      this.scaling.horizontal.cooldownPeriod
    ) {
    ...
    if (compositeLoad > this.scaling.horizontal.scaleUpThreshold) {
      if (
        this.clusterState.actualWorkers < this.scaling.horizontal.maxInstances
      ) {
        return {
          action: 'scale-up',
      ...
      if (
        this.clusterState.actualWorkers > this.scaling.horizontal.minInstances
      ) {
        return {
          action: 'scale-down',
  ...
  async scaleUp(reason) {
    ...
    this.logger.info(
      `Scaling up: ${reason} (${this.clusterState.actualWorkers} → ${targetWorkers})`
    );
  ...
  async scaleDown(reason) {
    ...
    this.logger.info(
      `Scaling down: ${reason} (${this.clusterState.actualWorkers} → ${targetWorkers})`
    );
  ...
  async scaleToTarget(targetWorkers) {
    ...
    if (targetWorkers > currentWorkers) {
      ...
      for (const workerId of workerIds) {
        ...
        if (worker) {
          this.logger.info(
            `Gracefully shutting down worker ${worker.process.pid}`
          );
          worker.disconnect();
    ...
    try {
      ...
      if (prediction.confidence > this.scaling.predictive.confidenceThreshold) {
        ...
        if (prediction.workload > this.scaling.horizontal.scaleUpThreshold) {
          this.logger.info(
            `Predictive scaling: High workload predicted (${Math.round(prediction.workload * 100)}%, confidence: ${Math.round(prediction.confidence * 100)}%)`
          );
  ...
  async healthCheckWorkers() {
    ...
    for (const [workerId, worker] of this.clusterState.workers) {
      if (worker.status === 'online') {
        ...
        const lastMetricsAge =
          now - (worker.lastMetricsUpdate || worker.onlineTime);
        if (lastMetricsAge > 180000) {
          // 3 minutes
          unhealthyWorkers.push(workerId);
          this.logger.warn(
            `Worker ${worker.pid} appears unresponsive (last metrics: ${Math.round(lastMetricsAge / 1000)}s ago)`
          );
        }

        // Check for memory leaks
        if (worker.metrics.memoryUsage > 1024 * 1024 * 1024 * 2) {
          // 2GB
          unhealthyWorkers.push(workerId);
          this.logger.warn(
            `Worker ${worker.pid} has high memory usage: ${Math.round(worker.metrics.memoryUsage / 1024 / 1024)}MB`
          );
        }
      }
    }
...
class WorkloadPredictor {
  ...
  async predictWorkload(metrics, lookAheadMinutes) {
    ...
    const predictedLoad = Math.max(
      0,
      Math.min(1, currentLoad + (workloadTrend * lookAheadMinutes) / 60)
    );
