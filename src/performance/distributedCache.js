...
class DistributedCache extends EventEmitter {
  ...
  async initialize() {
    try {
      ...
      this.logger.info(
        `Distributed Cache initialized with ${this.config.shardCount} shards`
      );
  ...
  async initializeDistributedNodes() {
    for (const nodeAddress of this.config.nodes) {
      try {
        ...
        this.logger.warn(
          `Failed to connect to cache node ${nodeAddress}:`,
          error
        );
      }
    }
  ...
  async get(key, options = {}) {
    ...
    try {
      ...
      if (this.config.enableDistribution && this.remoteNodes.size > 0) {
        const distributedResult = await this.getDistributed(
          validatedKey,
          options
        );
  ...
  async set(key, value, options = {}) {
    ...
    try {
      ...
      // Set in local cache
      await this.setLocal(validatedKey, entry.value, {
        ...options,
    ...
    for (const node of nodes) {
      promises.push(
        node
          ...
          .catch(error => {
            this.logger.warn(
              `Failed to delete from node ${node.address}:`,
              error
            );
          })
      );
    ...
    for (const node of this.remoteNodes.values()) {
      promises.push(
        node
          ...
          .catch(error => {
            this.logger.warn(
              `Failed to clear from node ${node.address}:`,
              error
            );
          })
      );
  ...
  selectNodesForKey(key) {
    ...
    for (
      let i = 1;
      i < this.config.replicationFactor && i < nodes.length;
      i++
    ) {
  ...
  async checkMemoryLimits() {
    ...
    const totalKeys = this.localShards.reduce(
      (sum, shard) => sum + shard.size,
      0
    );
  ...
  async syncWithRemoteNodes() {
    ...
    const healthPromises = Array.from(this.remoteNodes.values()).map(
      async node => {
        try {
          await node.ping();
    ...
    for (const result of healthResults) {
      if (result.status === 'fulfilled' && !result.value.healthy) {
        this.logger.warn(
          `Node ${result.value.node.address} is unhealthy:`,
          result.value.error
        );
      }
    }
  ...
  getStats() {
    ...
    const avgLatency =
      this.stats.totalLatency / (this.stats.hits + this.stats.misses) || 0;
    const totalKeys = this.localShards.reduce(
      (sum, shard) => sum + shard.size,
      0
    );
