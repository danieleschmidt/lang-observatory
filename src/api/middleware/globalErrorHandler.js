...
class GlobalErrorHandler {
    ...
    middleware() {
        return async (error, req, res, next) => {
            try {
                ...
                // Build response
                const response = await this.buildErrorResponse(error, classification, locale);
    ...
    classifyError(error) {
        ...
        // Database errors
        if (error.code && (error.code.startsWith('ER_') || error.code.includes('SQLITE'))) {
            return {
                type: 'database',
                status: 500,
                category: 'server',
