...
class ReliabilityManager extends EventEmitter {
  ...
  setupDefaultCircuitBreakers() {
    const defaultConfigs = [
      ...
      },
      {
        ...
        fallback: async () => ({
          success: true,
          message: 'Trace recorded locally',
        }),
      },
      {
        name: 'openlit',
        ...
        fallback: async () => ({
          success: true,
          message: 'Metrics cached locally',
        }),
      },
    ];
  ...
  createCircuitBreaker(name, config = {}) {
    if (this.circuitBreakers.has(name)) {
      this.logger.warn(
        `Circuit breaker "${name}" already exists, updating configuration`
      );
    ...
    try {
      ...
      if (fallback) {
        try {
          ...
          this.logger.error(
            `Fallback failed for operation "${name}":`,
            fallbackError
          );
          throw fallbackError;
        }
  ...
  async executeWithRetry(operation, retryConfig, context = {}) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        this.logger.warn(
          `Operation failed on attempt ${attempt + 1}, retrying in ${Math.round(nextDelay)}ms:`,
          error.message
        );
