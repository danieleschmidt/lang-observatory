...
class CircuitBreaker extends EventEmitter {
    ...
    async execute(operation, context = {}) {
        ...
        if (this.bulkheadEnabled && this.currentRequests >= this.maxConcurrentRequests) {
            this.metrics.totalRejections++;
            const error = new Error(`Circuit breaker "${this.name}": Too many concurrent requests`);
        ...
        if (this.state === 'OPEN') {
            if (this.shouldAttemptReset()) {
                ...
                this.logger.info(`Circuit breaker "${this.name}" attempting reset (HALF_OPEN)`);
                this.emit('stateChange', { name: this.name, state: 'HALF_OPEN' });
            } else {
                this.metrics.totalRejections++;
                const error = new Error(`Circuit breaker "${this.name}": Circuit is OPEN`);
                ...
                if (this.fallbackFunction) {
                    try {
                        ...
                        this.emit('fallbackExecuted', { name: this.name, result: fallbackResult });
                        return fallbackResult;
                    } catch (fallbackError) {
                        this.logger.error(`Fallback failed for "${this.name}":`, fallbackError);
                        this.emit('fallbackFailed', { name: this.name, error: fallbackError });
                    }
                }
    ...
    shouldTrip() {
        ...
        if (this.adaptiveThreshold) {
            ...
            threshold = Math.max(2, Math.floor(this.failureThreshold * (1 - recentFailureRate)));
        }
    ...
    trip(error) {
        ...
        this.trackStateTransition('OPEN', error.message || 'Failure threshold exceeded');
        
        this.logger.warn(`Circuit breaker "${this.name}" tripped to OPEN state`);
        this.emit('stateChange', { name: this.name, state: 'OPEN', error });
        this.emit('circuitOpened', { name: this.name, error, failures: this.failures });
    }
    ...
    forceOpen(reason = 'Manually forced open') {
        ...
        this.logger.warn(`Circuit breaker "${this.name}" forced to OPEN state: ${reason}`);
    ...
    forceClose(reason = 'Manually forced close') {
        ...
        this.logger.info(`Circuit breaker "${this.name}" forced to CLOSED state: ${reason}`);
    }
    ...
    getStatus() {
        return {
            ...
            requestsInWindow: this.getRequestsInWindow(),
            lastTransition: this.metrics.stateTransitions[this.metrics.stateTransitions.length - 1],
            nextAttempt: this.nextAttempt
        };
    }
