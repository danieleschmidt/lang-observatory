...
class HealthMonitor extends EventEmitter {
    ...
    this.registerHealthCheck('system', async () => {
      ...
      return {
        healthy:
          ...
          this.config.criticalThresholds.memory,
        metrics: {
  ...
  async recordHealthMetrics(componentName, result) {
    try {
      ...
      await Promise.all(
        metrics.map(metric =>
          this.metrics
            .recordCustomMetric(`health-${componentName}`, metric)
            .catch(error =>
              this.logger.warn('Failed to record health metric:', error)
            )
        )
      );
    } catch (error) {
  ...
  async checkAlerts(healthStatus) {
    ...
    for (const [component, result] of Object.entries(healthStatus.components)) {
      if (result.healthy) {
        ...
        if (
          this.alertsSent.has(criticalKey) ||
          this.alertsSent.has(warningKey)
        ) {
  ...
  startPeriodicChecks() {
    ...
    this.logger.info(
      `Started periodic health checks every ${this.config.checkInterval}ms`
    );
  }
  ...
  async analyze(healthHistory) {
    ...
    for (const componentName of componentNames) {
      const values = recent.map(
        entry => entry.components[componentName]?.successRate || 0
      );
...
class HealthAnomalyDetector {
  ...
  async detect(currentHealth, healthHistory) {
    ...
    for (const [componentName, result] of Object.entries(
      currentHealth.components
    )) {
      const historicalValues = recent
        .map(entry => entry.components[componentName]?.duration || 0)
        .filter(val => val > 0);
