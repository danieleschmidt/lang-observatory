...
class ThreatDetectionSystem extends EventEmitter {
    ...
    async analyzeRequest(req, metadata = {}) {
        ...
        try {
            ...
            // Rate limiting check
            const rateLimitResult = this.checkRateLimit(analysis.ip, analysis.userAgent);
    ...
    async analyzeBehavior(analysis) {
        ...
        if (profile) {
            ...
            if (analysis.method !== profile.lastMethod && profile.methodChanges > 10) {
                analysis.threats.push({
    ...
    updateBehaviorProfile(ip, analysis) {
        ...
        profile.methods.set(analysis.method, (profile.methods.get(analysis.method) || 0) + 1);
