...
class SchemaValidator {
  ...
  normalizeSchema(schema) {
    ...
    if (normalized.properties) {
      Object.keys(normalized.properties).forEach(key => {
        normalized.properties[key] = this.normalizeSchema(
  ...
  async validate(schemaName, data, options = {}) {
    ...
    try {
      ...
      this.logger.debug(
        `Validation completed for ${schemaName} in ${duration}ms`
      );
      ...
      const duration = Date.now() - startTime;
      this.logger.error(
        `Validation failed for ${schemaName} after ${duration}ms:`,
        error
      );
      throw error;
    }
  ...
  async validateAgainstSchema(data, schema, path = [], options = {}) {
    ...
    if (!this.validateType(data, schema.type)) {
      throw new ValidationError(
        `Invalid type at ${context.path}. Expected ${schema.type}, got ${typeof data}`
      );
    }
    ...
    if (schema.required && schema.type === 'object') {
      for (const field of schema.required) {
        if (data[field] === undefined || data[field] === null) {
          throw new ValidationError(
            `Missing required field: ${path.concat(field).join('.')}`
          );
        }
      }
    }
    ...
    if (schema.validate && typeof schema.validate === 'function') {
      ...
      if (!customResult.valid) {
        throw new ValidationError(
          `Custom validation failed at ${context.path}: ${customResult.message}`
        );
      }
    }
  ...
  validateType(data, expectedType) {
    ...
    if (Array.isArray(expectedType)) {
      return (
        expectedType.includes(typeof data) ||
        (expectedType.includes('null') && data === null)
      );
    }
    ...
    if (schema.additionalProperties === false) {
      ...
      for (const key of Object.keys(data)) {
        if (!allowedKeys.includes(key)) {
          throw new ValidationError(
            `Additional property not allowed: ${[...path, key].join('.')}`
          );
        }
      }
    }
  ...
  async validateArray(data, schema, path, options) {
    if (schema.maxItems && data.length > schema.maxItems) {
      throw new ValidationError(
        `Array too long at ${path.join('.')}. Max: ${schema.maxItems}, got: ${data.length}`
      );
    }

    if (schema.minItems && data.length < schema.minItems) {
      throw new ValidationError(
        `Array too short at ${path.join('.')}. Min: ${schema.minItems}, got: ${data.length}`
      );
    }

    if (data.length > this.config.maxArrayLength) {
      throw new ValidationError(
        `Array exceeds maximum length: ${this.config.maxArrayLength}`
      );
    }
  ...
  validateString(data, schema, context) {
    if (schema.minLength && data.length < schema.minLength) {
      throw new ValidationError(
        `String too short at ${context.path}. Min: ${schema.minLength}, got: ${data.length}`
      );
    }

    if (schema.maxLength && data.length > schema.maxLength) {
      throw new ValidationError(
        `String too long at ${context.path}. Max: ${schema.maxLength}, got: ${data.length}`
      );
    }

    if (schema.pattern && !new RegExp(schema.pattern).test(data)) {
      throw new ValidationError(
        `String doesn't match pattern at ${context.path}`
      );
    }
  ...
  validateNumber(data, schema, context) {
    if (schema.minimum && data < schema.minimum) {
      throw new ValidationError(
        `Number too small at ${context.path}. Min: ${schema.minimum}, got: ${data}`
      );
    }

    if (schema.maximum && data > schema.maximum) {
      throw new ValidationError(
        `Number too large at ${context.path}. Max: ${schema.maximum}, got: ${data}`
      );
    }

    if (schema.multipleOf && data % schema.multipleOf !== 0) {
      throw new ValidationError(
        `Number not multiple of ${schema.multipleOf} at ${context.path}`
      );
    }
  }
