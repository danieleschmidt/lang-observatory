...
// Record an LLM call
router.post('/calls', authMiddleware, validationMiddleware('llmCall'), async (req, res) => {
    try {
        const {
            traceId,
            sessionId,
...
router.get('/calls/trace/:traceId', authMiddleware, async (req, res) => {
    try {
        ...
        logger.error(`Failed to get LLM calls for trace ${req.params.traceId}:`, error);
...
router.get('/calls/session/:sessionId', authMiddleware, async (req, res) => {
    try {
        ...
        logger.error(`Failed to get LLM calls for session ${req.params.sessionId}:`, error);
        res.status(500).json({
            success: false,
            error: 'Internal Server Error',
            message: 'Failed to retrieve LLM calls'
...
router.get('/calls/provider/:providerId', authMiddleware, async (req, res) => {
    try {
        ...
        const calls = await llmCallRepo.findByProvider(parseInt(providerId), options);
        ...
        logger.error(`Failed to get LLM calls for provider ${req.params.providerId}:`, error);
        res.status(500).json({
            success: false,
            error: 'Internal Server Error',
            message: 'Failed to retrieve LLM calls'
...
router.get('/errors/recent', authMiddleware, async (req, res) => {
    try {
        ...
        const errors = await llmCallRepo.getRecentErrors(Math.min(parseInt(limit), 200));
...
router.post('/calls/batch', authMiddleware, validationMiddleware('llmCallBatch'), async (req, res) => {
    try {
        ...
        for (const [index, callData] of calls.entries()) {
            try {
                ...
                const result = await llmCallRepo.createWithProviderModel({
                    ...
                    startedAt: callData.startedAt ? new Date(callData.startedAt) : new Date(),
