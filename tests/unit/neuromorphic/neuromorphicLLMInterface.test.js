...
} = require('../../../src/neuromorphic/neuromorphicLLMInterface');
const {
  PhotonProcessor,
} = require('../../../src/neuromorphic/photonProcessor');
...
describe('NeuromorphicLLMInterface', () => {
    ...
    mockPhotonProcessor = {
      ...
      processLLMData: jest.fn().mockResolvedValue({
        ...
        insights: {
          ...
          quantumCorrelations: {
            strongCorrelations: 15,
            totalCorrelations: 50,
          },
        },
        processingTime: 100,
  ...
  describe('Initialization', () => {
    ...
    test('should handle initialization failure', async () => {
      mockPhotonProcessor.initialize.mockRejectedValue(
        new Error('Photon processor failed')
      );

      await expect(neuromorphicInterface.initialize()).rejects.toThrow(
        'Photon processor failed'
      );
    ...
    test('should setup event listeners correctly', async () => {
      ...
      expect(mockPhotonProcessor.on).toHaveBeenCalledWith(
        'neuronSpike',
        expect.any(Function)
      );
      expect(mockPhotonProcessor.on).toHaveBeenCalledWith(
  ...
  describe('Adaptive Models', () => {
    ...
    test('should generate different profiles for different providers', async () => {
      const openaiProfile =
        neuromorphicInterface.adaptiveModels.get('openai').neuromorphicProfile;
      const anthropicProfile =
        neuromorphicInterface.adaptiveModels.get(
          'anthropic'
        ).neuromorphicProfile;

      expect(openaiProfile.preferredWavelengths.primary).not.toBe(
        anthropicProfile.preferredWavelengths.primary
      );
    });
    ...
    test('should create adaptive model for unknown provider', async () => {
      ...
      const model =
        neuromorphicInterface.getOrCreateAdaptiveModel(unknownProvider);
      ...
      expect(neuromorphicInterface.adaptiveModels.has(unknownProvider)).toBe(
        true
      );
    });
  ...
  describe('LLM Call Processing', () => {
    ...
    test('should handle processing errors gracefully', async () => {
      ...
      await expect(
        neuromorphicInterface.processLLMCall(llmCallData)
    ...
    test('should throw error when not initialized', async () => {
      ...
      await expect(
        uninitializedInterface.processLLMCall(llmCallData)
  ...
  describe('Data Preparation and Quality Estimation', () => {
    ...
    test('should prepare LLM data for neuromorphic processing', () => {
      ...
      const prepared = neuromorphicInterface.prepareLLMDataForNeuromorphic(
        llmCallData,
        adaptiveModel
      );
    ...
    test('should get historical performance correctly', () => {
      ...
      const openaiPerf =
        neuromorphicInterface.getHistoricalPerformance('openai');
      const unknownPerf =
        neuromorphicInterface.getHistoricalPerformance('unknown');
    ...
    test('should generate recommendations based on confidence levels', async () => {
      ...
      mockPhotonProcessor.processLLMData.mockResolvedValue({
        insights: {
          performanceOptimization: {
            confidence: 0.9,
            recommendations: ['High perf rec'],
          },
          costEfficiencyRecommendations: {
            confidence: 0.8,
            recommendations: ['High cost rec'],
            costEfficiency: 0.9,
          },
          qualityImprovements: {
            confidence: 0.7,
            recommendations: ['High quality rec'],
          },
          predictiveAnalytics: {
            confidence: 0.85,
            predictions: [
      ...
      const predictiveRecs = recommendations.filter(
        r => r.type === 'predictive'
      );
  ...
  describe('Adaptive Model Updates', () => {
    ...
    test('should update adaptive model based on call data', async () => {
      ...
      const initialModel = {
        ...neuromorphicInterface.adaptiveModels.get('openai').baselineMetrics,
      };
      ...
      await neuromorphicInterface.updateAdaptiveModel(
        'openai',
        llmCallData,
        mockNeuromorphicResult
      );
    ...
    test('should normalize adaptive weights after update', async () => {
      ...
      const weights =
        neuromorphicInterface.adaptiveModels.get('openai').adaptiveWeights;
  ...
  describe('Insights Retrieval', () => {
    ...
    test('should retrieve LLM insights by call ID', async () => {
      ...
      const insights =
        await neuromorphicInterface.getLLMInsights('test-call-1');
    ...
    test('should return undefined for unknown call ID', async () => {
      const insights =
        await neuromorphicInterface.getLLMInsights('unknown-call');
    ...
    test('should get provider insights correctly', async () => {
      ...
      const openaiInsights = await neuromorphicInterface.getProviderInsights(
        'openai',
        10
      );
    ...
    test('should summarize provider insights correctly', () => {
      const mockInsights = [
        {
          ...
          adaptiveRecommendations: {
            ...
            recommendations: [{ type: 'performance' }, { type: 'cost' }],
          },
        ...
        },
        {
          ...
          adaptiveRecommendations: {
            ...
            recommendations: [{ type: 'performance' }, { type: 'quality' }],
          },
      ...
      const summary =
        neuromorphicInterface.summarizeProviderInsights(mockInsights);
  ...
  describe('Metrics and Health Monitoring', () => {
    ...
    test('should report health status correctly', async () => {
      ...
      expect(health.components).toHaveProperty('photonProcessor');
    ...
    test('should calculate efficiency metrics', () => {
      ...
      const efficiency =
        neuromorphicInterface.calculateNeuromorphicEfficiency();
  ...
  describe('Error Handling and Edge Cases', () => {
    ...
    test('should handle empty insights gracefully', () => {
      const emptyInsights = [];
      const summary =
        neuromorphicInterface.summarizeProviderInsights(emptyInsights);
    ...
    test('should handle queue processing errors gracefully', async () => {
      ...
      realTimeInterface.processLLMCallInternal = jest
        .fn()
  ...
  describe('Event Handling', () => {
    ...
    test('should emit llmCallProcessed events', done => {
      ...
      neuromorphicInterface.processLLMCall({
        id: 'test-event-call',
        provider: 'openai',
      });
    });
    ...
    test('should handle neuromorphic activity events', done => {
      ...
      const spikeHandler = mockPhotonProcessor.on.mock.calls.find(
        call => call[0] === 'neuronSpike'
      )[1];
