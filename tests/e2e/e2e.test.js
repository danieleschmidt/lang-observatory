...
describe('End-to-End Tests', () => {
  ...
  afterAll(async () => {
    try {
      execSync(`helm uninstall ${releaseName} -n ${testNamespace}`, {
        stdio: 'inherit',
      });
      execSync(`kubectl delete namespace ${testNamespace}`, {
        stdio: 'inherit',
      });
    } catch (error) {
  ...
  test('Chart can be installed in test environment', async () => {
    ...
    expect(() => {
      execSync(
        `helm install ${releaseName} ${chartPath} -n ${testNamespace} --wait --timeout=300s`,
        {
          stdio: 'inherit',
          cwd: __dirname,
        }
  ...
  test('All pods are running', () => {
    const output = execSync(`kubectl get pods -n ${testNamespace} -o json`, {
      encoding: 'utf8',
    });
  ...
  test('Services are accessible', () => {
    const output = execSync(
      `kubectl get services -n ${testNamespace} -o json`,
      { encoding: 'utf8' }
    );
  ...
  test('Health checks pass', () => {
    expect(() => {
      execSync(`helm test ${releaseName} -n ${testNamespace}`, {
        stdio: 'inherit',
      });
    }).not.toThrow();
  });
